...
- **`family/:fid/*`**：`App.tsx` の `FamilyGuard`（`Outlet` 方式の共通親ルート）が `store.families[fid]` を確認し、未知なら `<Navigate to="/home" replace />`。共有リンクのタイポや削除済 id で空画面に着地する事故を防ぐ。
- **`memory/:mid/edit`**：`MemoryEditorPage` で `canViewMemory(existing, currentViewerPersonId)` を検査。**書き手＋閲覧者に登録された人なら全員編集可**。それ以外にはガード画面（「書き手と閲覧者に登録された人のみ編集できます／自分を切り替える」）を返す。新規作成（`mid` 無し）はガード対象外。ガード画面の分岐は hook 群の後で行う（rules-of-hooks 遵守）。
